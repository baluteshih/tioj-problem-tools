...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    switch (result) {
        ...
        case _points:
            printf("0\n");
            printf("SPECJUDGE_OVERRIDE_SCORE %lf\n", __testlib_points * 100);
            if (__testlib_points + 1e-6 < 1)
                printf("SPECJUDGE_OVERRIDE_VERDICT WA\n");
            errorName = "points ";
...
void registerTestlibCmd(int argc, char *argv[]) {
    ...
    if (argc < 4 || argc > 7) {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
                    std::string("[--testset testset] [--group group] <output-file> <input-file> <answer-file> <language> <source-code-path> <stage-number>") +
                    "\nUse \"--help\" to get help information");
    }
    ...
    ouf.init(args[1], _output);
    ouf.skipBom();
    inf.init(args[2], _input);
    ans.init(args[3], _answer);

    if (argc > 4) language = args[4];
    if (argc > 5) source.init(args[5], _output);
    if (argc > 6) current_stage_number = stoi(args[6]);
}
